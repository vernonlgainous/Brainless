...
public class Weapon extends Item {
	...
	float attackTimer = 0; // Timer to see if we can fire shot again, counts
							// down, 1 sec = 60 frames
	int ammoRemaining;
	final int constAmmoInClip; // keep track of original number of bullets in
								// clip
	int ammoInClip; // changes as the gun is fired
	int numberOfClips;
	int reloadDuration;
	float reloadTimer; // Timer to see if reload is complete, counts up, 1 sec =
						// 60 frames
	int weaponDamage;
	...
	public void update(Vector2 playerPosition) {
		...
		for (int i = bullets.size() - 1; i >= 0; i--) {
			...
			if (temp.position.X < -temp.rect.width || temp.position.X > 800
					|| temp.position.Y > 480
					|| temp.position.Y < -temp.rect.height)
				bullets.remove(temp);
		}
	}
